...
public sealed partial class SecretsView : UserControl, IAdminView
{
    ...
    {
        if (Selected is not { } summary) { Fail("Önce bir parola seçin."); return; }
        if (MessageBox.Show($"'{summary.Title}' parolası silinsin mi? İlgili erişim izinleri de kalkar.",
                "Parola sil", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes)
            return;
        try
        {
