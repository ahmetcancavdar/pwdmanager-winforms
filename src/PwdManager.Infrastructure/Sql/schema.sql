...
-- ---------------------------------------------------------------------------
-- categories
--   deleted_at NULL  -> aktif; dolu -> "Silinenler" (soft delete). Ad benzersizliği
--   yalnız aktif kayıtlar arasında uygulama katmanında zorlanır (bkz. CategoryService).
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `categories` (
  ...
  CONSTRAINT `fk_categories_deleter` FOREIGN KEY (`deleted_by`)
    REFERENCES `users` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB;
...
--   Etkin görünürlük = secrets.deleted_at IS NULL AND categories.deleted_at IS NULL
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- secret_denies: a carve-out. When a personnel has an ENTIRE category granted,
-- a row here hides one specific secret inside it from that personnel.
-- Effective access = (category granted OR secret granted) AND NOT denied.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `secret_denies` (
  `user_id`   BIGINT NOT NULL,
  `secret_id` BIGINT NOT NULL,
  `denied_by` BIGINT NULL,
  `denied_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`user_id`, `secret_id`),
  CONSTRAINT `fk_sd_user` FOREIGN KEY (`user_id`)
    REFERENCES `users` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_sd_secret` FOREIGN KEY (`secret_id`)
    REFERENCES `secrets` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_sd_denier` FOREIGN KEY (`denied_by`)
    REFERENCES `users` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB;
